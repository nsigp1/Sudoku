...
def creer(nombres: int) -> Sudoku:
    ...
    if nombres > 50: #evite de causer des problemes de lenteur, quitte a sacrifier de l'aleatoire (on peut remplacer 50 par une autre valeur, plus grande pour plus d'aleatoire, moins pour plus de rapidite, 50 etant un bon compromis)
        S = creer(50)
        S.resoudre()
        while nombres < 81:
            indice = randint(0, 80)
...
class Fenetre:
    ...
    def __init__(self, win):
        ...
        self.win.minsize(600, 400) #format minimum de la fenetre
    ...
    def page_depart(self, texte = False):
        ...
        if texte: #si l'utilisateur a resolu un Sudoku
            texte_felicitations = tk.Label(self.win, text="Bravo !", font=("Arial", 30)).pack(side=tk.TOP, pady=30)
        
        frame = tk.Frame(self.win) #Frame pour la mise en page
        btn_creer = tk.Button(frame, text="Créer une grille", command=self.page_creer1, font=("Arial", 30)).pack(pady=80) #creer un Sudoku
        btn_resoudre = tk.Button(frame, text="Résoudre une grille", command=self.page_resoudre, font=("Arial", 30)).pack(pady=80) #resoudre un Sudoku
    ...
    def page_creer1(self):
        ...
        frame = tk.Frame(self.win) #Frame pour la mise en page
        txt_selection = tk.Label(frame, text="Combien de chiffres préremplis voulez-vous dans la grille ?\n(Attention, plus le nombre de chiffres demandés est élevé, plus ce sera long)", font=("Arial", 25)) #texte
    ...
    def creation_grille(self):
        ...
        if int(self.choix.get()) >= 0 and int(self.choix.get()) <= 81: #si le chiffre est bon (entre 0 et 81 inclus)
            self.page_creer2(int(self.choix.get()))
        else:
            txt_mauvaise_selection = tk.Label(self.win, text="Merci de faire une entrée valide (un nombre entre 0 et 81 inclus)", font=("Arial", 25), fg="red").pack() #texte fausse entree
        ...
        for i in range(9):
            for j in range(9):
                if self.Sudo[i*9 + j].val == None: #case vide
                    ...
                    self.cases[str(i) + str(j)] = tk.Entry(self.frame, width=2, font=("Arial", 30), validate="key", validatecommand=vcmd)
                    self.cases[str(i) + str(j)].grid(row=i, column=j) #cree une entree utilisateur selon les regles de validate()
                else:
                    tk.Label(self.frame, width=2, font=("Arial", 30), text=str(self.Sudo[i*9 + j].val)).grid(row=i, column=j) #remplit la case avec le chiffre prerempli
                    
        btn_verifier = tk.Button(self.frame, text="Verifier",font=("Arial", 30), command=self.verifier_grille).grid(row=10, column=3, columnspan=3, pady=20) #bouton Verifier
    ...
    def validate(self, action, index, valeur, ancienne_valeur, texte, type_entree, type_declencheur, nom_widget):
        ...
        if action == "1": #il y a un changement dans l'entree
            if texte in '123456789' and len(valeur) < 2: #entree permise
                return True
    ...
    def page_resoudre(self):
        ...
        btn_resoudre = tk.Button(self.frame, text="Résoudre la grille",font=("Arial", 30), command=self.resoudre_grille).grid(row=10, column=0, columnspan=9, pady=20) #bouton Verifier
        ...
        for i in range(9):
            for j in range(9):
                if grille[i*9 + j].fixe == True: #l'utilisateur a renseigne la case
                    tk.Label(self.frame, width=2, font=("Arial", 30), text=str(grille[i*9 + j].val)).grid(row=i, column=j) #cree une case avec le chiffre de l'utilisateur
                else:
                    tk.Label(self.frame, width=2, font=("Arial", 30), text=str(grille[i*9 + j].val), fg="red").grid(row=i, column=j) #cree une case avec le chiffre inscrit par l'appel resoudre_grille()
